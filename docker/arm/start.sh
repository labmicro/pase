#/bin/sh

# rm -f agent-key agent-key.pub
# ssh-keygen -f agent-key -m PEM -t rsa -N ""
pub_key="ssh-rsa "

docker build --tag labmicro/testing-agent:lts .
docker run -d --restart=on-failure --name "jenkins.arm" --privileged -v /dev/bus/usb:/dev/bus/usb -v /dev/ttyUSB1:/dev/ttyUSB1 labmicro/testing-agent:lts "$pub_key"
